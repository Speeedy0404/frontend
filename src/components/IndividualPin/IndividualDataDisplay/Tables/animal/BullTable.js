...
const BullTable = ({ key, gpp, dataBull, additionalParam, onSelectedChange }) => {
    ...
    const columns = useMemo(() => [
        ...
        { Header: 'Компл.', accessor: 'kompleks'},
        { Header: 'Семя', accessor: 'sperma', className: 'sperma'  },
        { Header: `RM (${avg.milk ? getValue_OrDefault(avg.milk.avg_rm) : '-'})`, accessor: d => d.milkproductionindexbull ? getValue_OrDefault(d.milkproductionindexbull.rm) : '', id: 'rm' },
        ...
        { Header: `RBVU (${avg.conf ? getValue_OrDefault(avg.conf.avg_rbvu) : '-'})`, accessor: d => d.conformationindexbull ? getValue_OrDefault(d.conformationindexbull.rbvu) : '', id: 'rbvu' , },
